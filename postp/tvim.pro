...
; !p.multi=[0,2,2]
; loadct,5
; tvim,im,/scale,range=[immn,immx],color=findrng(0,255,4)
; tvim,im,/scale,range=[immn,immx],color=findrng(0,255,5)
; tvim,im,/scale,range=[immn,immx],color=findrng(0,255,8)
; tvim,im,/scale,range=[immn,immx],color=findrng(0,255,10)
;;;
;;;
...
px=fix(!x.window*!d.x_vsize)
py=fix(!y.window*!d.y_vsize)
xsize=px(1)-px(0)
ysize=py(1)-py(0)
...
if keyword_set(scale) or keyword_set(rmarg) then begin
  ...
  xmarg=fix(!d.x_ch_size*(4+4*lcharsize))
  if keyword_set(stitle) then xmarg=fix(xmarg+2*!d.y_ch_size*lcharsize)
  xsize=fix(xsize-rmarg*xmarg)
endif
oxs=xsize
oys=ysize
if keyword_set(aspect) eq 0 then aspect=float(nx)/ny
if xsize gt ysize*aspect then xsize=fix(ysize*aspect)  $
                         else ysize=fix(xsize/aspect )
